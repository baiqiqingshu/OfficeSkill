...
def main() -> int:
    ...
    for output, theme in [(light, args.light_theme), (dark, args.dark_theme)]:
        if render(d2, args.input, output, theme, args.layout, args.animate_interval, bundle):
            continue
        if args.fallback_layout != args.layout:
            print(f"Retrying with {args.fallback_layout}")
            if render(d2, args.input, output, theme, args.fallback_layout, args.animate_interval, bundle):
                continue
        status = 1
